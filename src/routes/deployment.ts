...
const mapDeploymentToResponse = ({ metadata }: V1Deployment) => ({
  ...
  host: `${metadata?.labels?.[`${strings.ISOLATED_CHALLENGE_QUALIFIER}/deployment`]
    }.${BASE_DOMAIN}`,
...
export default async function register(fastify: FastifyInstance) {
  ...
  fastify.addSchema({
    $id: 'challenge-manager.ctfpilot.com/schema/deployments.json',
    type: 'object',
    properties: {
      options: {
        type: 'object',
  ...
  fastify.route<{ Params: { name: string } }>({
    ...
    schema: {
      params: {
        name: {
          $ref: 'challenge-manager.ctfpilot.com/schema/deployments.json#/properties/name',
        },
      },
    },
    handler: async (request, reply) => {
  ...
  fastify.route<{
    ...
    schema: {
      body: {
        $ref: 'challenge-manager.ctfpilot.com/schema/deployments.json#/properties/options',
      },
      params: {
        name: {
          $ref: 'challenge-manager.ctfpilot.com/schema/deployments.json#/properties/name',
